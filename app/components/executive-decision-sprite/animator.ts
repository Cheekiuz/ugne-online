...
/** Public folder + optional GitHub Pages / Next basePath (`NEXT_PUBLIC_BASE_PATH`). */
const SPRITE_BASE = `${(process.env.NEXT_PUBLIC_BASE_PATH ?? "").replace(/\/$/, "")}/executive-decision-sprites`;

export const charSheet: Sheet[][] = [
  [
...
export class SpriteAnimator {
  ...
  constructor(
    ...
    this.containerEl.classList.add("edm-container");
  ...
  private updateSprite() {
    ...
    if (this.currentFrame >= this.totalFrames) {
      ...
      if (this.playingClick) {
        ...
        this.containerEl.classList.remove("waiting");
...
export function createSpriteAnimator(
  ...
  return new SpriteAnimator(mount, charSheet, sheetIndex, 320, 320, 5, 24, 121);
}
